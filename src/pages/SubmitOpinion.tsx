...
export default function SubmitOpinion() {
  ...
  const filteredBusinesses = mockBusinesses.filter(business => business.name.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const renderBusinessSelection = () => <div className="space-y-4">
      ...
      <SearchBar placeholder="Enter business name..." onSearch={setSearchQuery} />

      <div className="space-y-3">
        {filteredBusinesses.map(business => <Card key={business.id} className={`cursor-pointer transition-all ${selectedBusiness?.id === business.id ? 'ring-2 ring-primary bg-primary/5' : 'hover:shadow-md'}`} onClick={() => setSelectedBusiness(business)}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                {selectedBusiness?.id === business.id && <Check className="w-5 h-5 text-primary" />}
              </div>
            </CardContent>
          </Card>)}
      </div>

      {filteredBusinesses.length === 0 && searchQuery && <div className="text-center py-8">
  ...
  const renderTypeSelection = () => <div className="space-y-4">
      <div className="text-center space-y-2">
      ...
      <div className="space-y-3">
        ...
        return <Card key={type.id} className={`cursor-pointer transition-all ${opinionType === type.id ? 'ring-2 ring-primary bg-primary/5' : 'hover:shadow-md'}`} onClick={() => setOpinionType(type.id)}>
              <CardContent className="p-4">
  ...
  const renderDetailsForm = () => <div className="space-y-4">
      <div className="text-center space-y-2">
      ...
      <div className="space-y-4">
        <div>
          ...
          <Textarea placeholder="Describe what happened..." value={story} onChange={e => setStory(e.target.value)} maxLength={280} className="min-h-[120px]" />
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <input type="checkbox" id="allowContact" checked={allowContact} onChange={e => setAllowContact(e.target.checked)} className="w-4 h-4" />
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="anonymous" checked={remainAnonymous} onChange={e => setRemainAnonymous(e.target.checked)} className="w-4 h-4" />
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="private" checked={resolvePrivately} onChange={e => setResolvePrivately(e.target.checked)} className="w-4 h-4" />
  ...
  const renderProofUpload = () => <div className="space-y-4">
      <div className="text-center space-y-2">
  ...
  const renderRating = () => <div className="space-y-4">
      <div className="text-center space-y-2">
      ...
      <div className="grid grid-cols-3 gap-4">
        {ratingEmojis.map(rating => <Card key={rating.value} className={`cursor-pointer transition-all text-center ${selectedRating === rating.value ? 'ring-2 ring-primary bg-primary/5' : 'hover:shadow-md'}`} onClick={() => setSelectedRating(rating.value)}>
            <CardContent className="p-4">
              <div className="text-3xl mb-2">{rating.emoji}</div>
              <p className="text-center text-xs font-light">{rating.label}</p>
            </CardContent>
          </Card>)}
      </div>
    </div>;
  const renderFinalReview = () => <div className="space-y-4">
      <div className="text-center space-y-2">
  ...
  const getStepContent = () => {
    switch (step) {
      case "business":
        return renderBusinessSelection();
      case "type":
        return renderTypeSelection();
      case "details":
        return renderDetailsForm();
      case "proof":
        return renderProofUpload();
      case "rating":
        return renderRating();
      case "final":
      ...
      <div className="centered-container">
        ...
        <div className="flex-1 px-4 py-6 space-y-6">
          ...
          <div className="progress-container">
            <div className="flex items-center gap-2">
              {["business", "type", "details", "proof", "rating", "final"].map((stepName, index) => <div key={stepName} className="progress-step flex items-center">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${["business", "type", "details", "proof", "rating", "final"].indexOf(step) >= index ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}`}>
                    {index + 1}
                  </div>
                  {index < 5 && <div className={`w-8 h-1 mx-1 ${["business", "type", "details", "proof", "rating", "final"].indexOf(step) > index ? 'bg-primary' : 'bg-gray-200'}`} />}
                </div>)}
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            {step !== "business" && <Button variant="outline" onClick={handleBack} className="flex-1">
                Back
              </Button>}
